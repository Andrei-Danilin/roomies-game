...
**Consequences:** Requires an extra Route Handler (`/api/export-subscribers`) and a decision on where the CSV backup actually lands. MailerLite API key must stay server-side only.
**Amendment (2026-07-05):** The capture half shipped first — `POST /api/notify` (`lib/mailerlite.ts`) adds subscribers to MailerLite. At this point the weekly export/backup job didn't exist yet, so MailerLite was temporarily the only copy of the waitlist — the exact thing this ADR's "not the system of record" decision wanted to avoid. Accepted as a short-lived incremental step for a low-volume pre-launch waitlist.
**Resolved (2026-07-05):** Backup destination decided (Google Sheet, not cloud storage) and the export job shipped: `/api/export-subscribers` runs weekly via Vercel Cron (`vercel.json`), pulls the full MailerLite group via `lib/mailerlite.ts`'s `listGroupSubscribers`, and appends any subscriber not already present to the Sheet via `lib/google-sheets.ts` (service-account JWT auth, direct REST calls — no `googleapis` SDK dependency). MailerLite is no longer the sole copy of the waitlist.
...
**Date:** 2026-07-05
**Status:** Superseded by ADR-008 (2026-07-06)
...
## ADR-008: Language switching — locale-prefixed routes (/en, /es, /ru), full server rendering per locale
**Date:** 2026-07-06
**Status:** Accepted
**Context:** Issue #17 (epic #14, SEO/AI-crawler readiness) found that ADR-006's client-side Context meant Spanish/Russian content was never server-rendered or reachable at its own URL — crawlers only ever saw the English SSR snapshot, making `content/es.json`/`content/ru.json` invisible to search and AI answer engines. ADR-006 itself flagged this exact tradeoff and named locale-prefixed routing as the fallback. Considered keeping English unprefixed at `/` vs. prefixing all three locales uniformly — the unprefixed option would require a single root layout's `<html lang>` to vary per-route without a second `<html>` tag (App Router only allows one, defined in the true root layout), meaning the attribute would have to be set client-side after hydration, leaving the SSR'd `<html lang>` briefly wrong for non-default locales.
**Decision:** Prefix every locale (`/en`, `/es`, `/ru`), each fully server-rendered and statically generated (`generateStaticParams`). The root layout moves to `app/[locale]/layout.tsx` (no separate `app/layout.tsx`), matching Next.js's own documented i18n pattern. `proxy.ts` (Next 16's renamed `middleware` convention) 307-redirects bare `/` to the visitor's preferred locale, read from a `NEXT_LOCALE` cookie set by `LanguageSwitcher` on click, defaulting to `en`. `app/sitemap.ts` lists all three locale URLs with `hreflang` alternates (plus `x-default` → `/en`); `generateMetadata` sources its `description` from the actual locale's content (`getContent(locale).hero.pitch`) instead of a hardcoded English string.
**Consequences:** Un-amends ADR-001/ADR-006 back toward the original Server/Client split — `app/[locale]/page.tsx` and `components/HomeView.tsx` are Server Components again; only the language switcher, notify form, and FAQ accordion remain client components. `lib/locale-context.tsx` (React Context + `localStorage`) is deleted entirely — locale is now a routing concern, not client state. Bare `/` always issues a redirect (never indexed directly; `sitemap.ts` never lists it), so a cookie-less crawler lands on `/en` in one hop — a standard, Google-documented pattern.
